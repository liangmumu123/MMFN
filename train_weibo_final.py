...
def preprocess_data():
    ...
    for split, csv_path in [('train', 'E:/MMFN-data/data/weibo/train_weibo_final3.csv'), 
        ...
        cache_file = os.path.join(data_root, f'{split}_preprocessed_half.pkl')
        
        if os.path.exists(cache_file):
            print(f"✅ {split}_preprocessed_half.pkl 已存在，跳过预处理")
            continue
        ...
        for idx in tqdm(range(len(df)), desc=split):
            ...
            try:
                ...
                # 转换为半精度节省显存和传输时间
                swin_feat = swin_feat.half()
                
            except:
                swin_feat = torch.zeros(3, 224, 224, dtype=torch.float16)
...
def train():
    ...
    print("\n📊 加载预处理数据...")
    train_dataset = PreprocessedWeiboDataset('E:/MMFN-data/data/weibo/train_preprocessed_half.pkl')
    test_dataset = PreprocessedWeiboDataset('E:/MMFN-data/data/weibo/test_preprocessed_half.pkl')
    ...
    for epoch in range(epochs):
        ...
        for batch in train_bar:
            ...
            img_swin = img_swin.to(device, non_blocking=True)
