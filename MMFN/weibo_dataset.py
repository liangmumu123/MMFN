...
# Load a feature extractor from the transformers library 从transformers库加载特征提取器，提取图像特征
...
class weibo_dataset(data.Dataset):
    # 改：def __init__(self, root_path='/data/ymzhou/weibo', is_train=True):
    def __init__(self, root_path='D:/PyCharm/projects/MMFN/MMFN_yyt-master/data/weibo', is_train=True):
        ...
        # 改：self.local_path = '/data/ymzhou/dataset'
        self.local_path = 'D:/PyCharm/projects/MMFN/MMFN_yyt-master/data'

        # Read CSV file to populate label_dict
        # 改：wb = pandas.read_csv(self.local_path + '/{}_weibov.csv'.format('train' if is_train else 'test'))
...
# 将多个样本组合成一个批次。
# 使用 BertTokenizer 对文本进行编码。
# 使用 clip.tokenize 对文本进行 CLIP 模型的编码。
# 返回批次数据，包括输入 ID、注意力掩码、token 类型 ID、图像特征、CLIP 图像特征、CLIP 文本特征和标签。